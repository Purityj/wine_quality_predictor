...
This project implements a Random Forest classifier to predict wine quality based on physicochemical properties. Using the Wine Quality dataset from the UCI Machine Learning Repository (6,497 observations with 11 features, reduced to 5,320 after removing duplicates), we develop a robust prediction model that leverages the ensemble learning capabilities of Random Forest. Our analysis demonstrates that Random Forest effectively handles the non-linear relationships between chemical properties and wine quality, achieving an accuracy of approximately 74.6% on the test set. The model identifies alcohol content, volatile acidity, and density as the most influential factors in determining wine quality. A limitation of this study is that it only considers Portuguese "Vinho Verde" wines, which may limit the generalizability of the model to other wine types. This work provides valuable insights for wine producers to optimize production processes and maintain consistent quality standards.
...
- **Subtotal**: 6,497 observations
- **Total**: 6,497 observations (5,320 after removing duplicates)
...
To begin our analysis, we downloaded the two wine quality datasets provided by Cortez et al. (2009). These files contain measurements for red and white wine observations, including physicochemical properties and expert-assigned quality scores.
...
# Path to the image
img_path = "../results/figures/quality_distributions.png"

# Open and plot the image
img = Image.open(img_path)
plt.imshow(img)
plt.axis("off")  # Hide axes
plt.title("Wine Quality Distribution Analysis")  # Optional, caption is already via fig-cap
plt.show()
```
...
#| fig-cap: "Feature Correlations with Wine Quality"
#| fig-align: center
#| fig-width: 6
#| fig-height: 4
...
# Path to the image
img_path = "../results/figures/feature_correlations.png"

# Open and plot the image
img = Image.open(img_path)
plt.imshow(img)
plt.axis("off")  # Hide axes
plt.title("Feature Correlations with Wine Quality")  # Optional, caption is already via fig-cap
plt.show()
```
...
# Path to the image
img_path = "../results/figures/correlation_heatmap.png"

# Open and plot the image
img = Image.open(img_path)
plt.imshow(img)
plt.axis("off")  # Hide axes
plt.title("Feature Correlation Heatmap")  # Optional, caption is already via fig-cap
plt.show()
...
Note on class-specific metrics: Some classes, particularly the very high-quality wines, have very few observations. During evaluation, metrics such as precision and recall may be undefined for these classes because the model predicted zero observations in them. This does not affect the overall accuracy or the main performance conclusions of the model.
...
Due to the natural class imbalance in the wine-quality dataset, some classes have very few observations, which results in undefined precision and recall values (these appear as warnings during evaluation). Despite this, the evaluation helps identify where the model performs well and where predictions remain challenging.
...
if os.path.exists(report_path):
    with open(report_path, "r") as f:
        for line in f:
            ...
            if (
                not line
                or line.startswith("Random Forest")
                or line.startswith("precision")
                or line.startswith("accuracy")
                or line.startswith("macro")
                or line.startswith("weighted")
            ):
                continue

            parts = line.split()

            # Class labels have two tokens: e.g. "High (8-9)"
            if len(parts) == 6:
                class_name = f"{parts[0]} {parts[1]}"
                precision, recall, f1, support = parts[2:]
    ...
    df_report = pd.DataFrame(
    {"Error": [f"Classification report not found at {report_path}"]}
    )
...
The weighted averages are high because most observations belong to the Medium and Low classes, which the model predicts effectively.
...
The distribution of wine quality categories in the training and test sets is shown in Tables @tbl-train_dist_df and @tbl-test_dist_df (created during the data preprocessing stage). These tables summarize both the number of observations and the percentage of each quality class in the respective datasets.
...
#| fig-cap: "Confusion Matrix for Random Forest Optimized"
#| fig-align: center
#| fig-width: 6
#| fig-height: 4
...
# Path to the image
img_path = "../results/evaluation/confusion_matrix_random_forest_optimized.png"

# Open and plot the image
img = Image.open(img_path)
plt.imshow(img)
plt.axis("off")  # Hide axes
plt.title("Confusion Matrix for Random Forest Optimized")  # Optional, can match fig-cap
plt.show()
```
...
### Alcohol Content as Primary Quality Indicator

Our analysis identified alcohol content as the strongest predictor of wine quality (importance: 0.1466), which aligns with established oenological research. Higher alcohol levels are associated with increased body, flavor complexity, and perceived quality in wine evaluation [@jackson2014wine]. This finding supports the industry understanding that ethanol content serves as a proxy for grape ripeness and sugar conversion efficiency during fermentation [@ribereau2006handbook].

### Role of Acidity in Wine Quality

Volatile acidity demonstrated the second-highest feature importance (0.1049) with a strong negative correlation to quality. Elevated volatile acidity, primarily acetic acid, indicates bacterial spoilage and oxidation [@waterhouse2016understanding]. The negative relationship confirms that wines with higher acetic acid content receive lower quality ratings, consistent with findings on wine stability and sensory perception [@ribereau2006handbook].

### Density and Quality Relationships

Density emerged as the third most important feature (0.1046), exhibiting a negative correlation with quality. This relationship reflects the inverse association between alcohol content and density—higher quality wines with elevated alcohol have lower density values [@jackson2014wine]. This physicochemical principle has been documented in wine production literature [@waterhouse2016understanding].
...
3. **Class Imbalance**: High-quality wines (8-9) represent only ~3% of observations; model tuning with balanced class weights helped reduce bias toward majority classes.
4. **Future Directions**: 
...
Conduct systematic benchmarking against Logistic Regression, SVM, and Gradient Boosting to validate Random Forest's performance advantages
